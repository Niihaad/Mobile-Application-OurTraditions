...
class RemoveUser : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fun showSnackbarWithUndo(message: String, action: () -> Unit) {
            val rootView = findViewById<View>(android.R.id.content)
        ...
        Add_user2.setOnClickListener {
            if (password.text?.isBlank() == true || email.text?.isBlank() == true) {
                showSnackbarRed("Remplir tous les champs!")
            } else {
                if(db.removeUser(email.text.toString(),password.text.toString()))
                {
                showSnackbarWithUndo("Suppression de l' utilisateur avec succès") {
                    val user = User(nom.text.toString(),prenom.text.toString(),email.text.toString(),password.text.toString(),"","normalUser")
                   if( db.addAuth((user)))
                    showSnackbar("Suppression d'utilisateur annulée")
                    else
                    ...
                    showSnackbarRed("Cet utilisateur n'existe pas")
                }
            }
        }
    }
